...
def get_scores(lines):
    #pattern = r"Card \d+: (\d+(?:\s+\d+)*)\s*\|\s*(\d+(?:\s+\d+)*)"
    #pattern = r"Card \d+: ([\d\s]+)\s*\|\s*([\d\s]+)"
    ...
    for line in lines:
        ...
        if match:
            ...
            winning_string = match.group(1)
            player_string = match.group(2)
            ...
            score = int(2**(len(winning_numbers) - 1))
            scores.append(score)
...
def main(file_path):
    ...
    scores2 = get_scores_2(lines)
    
    assert scores == scores2
    
    total_sum = sum(scores)
            
    if 'input1.txt' in file_path: assert total_sum == 13
    if 'input2.txt' in file_path: assert total_sum == 27454 
