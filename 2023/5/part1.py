...
def map_values_mem_efficient(seeds, lines):
    maps = {}
    for i, line in enumerate(lines[1::]):
        for name in MAP_NAMES:
            if line.startswith(name):
    ...
    for seed in seeds:
        category = seed
        category_mapped_values['seeds'].append(category)
        for key, values in maps.items():
            for rule in values:
                dest_start, source_start, length = rule

                if source_start <= category and category < source_start + length:
                    category = category + dest_start - source_start
                    category_mapped_values[key].append(category)
                    print(key, category)
                    break;
            else:
                category_mapped_values[key].append(category)
...
def main(file_path):
    ...
    categories = map_values_mem_efficient(seeds, lines[1::])
